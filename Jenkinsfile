        ...
        stage('Checkout') {
            steps {
                sh """
                    ...
                    -d '{"text": "🚀 *NeedApp BE - Bat dau deploy*\\nBuild: #${BUILD_NUMBER}\\nBranch: ${GIT_BRANCH}\\nXem log: ${JENKINS_URL_PUBLIC}/job/needapp-be/${BUILD_NUMBER}/console"}'
                """
                checkout scm
            }
        }
    ...
    post {
        success {
            sh """
                curl -s -X POST '${GG_CHAT_WEBHOOK}' \
                -H 'Content-Type: application/json' \
                -d '{"text": "✅ *NeedApp BE Deploy THANH CONG*\\nBuild: #${BUILD_NUMBER}\\nCommit: ${GIT_COMMIT.take(7)}\\nURL: http://localhost:8081\\nXem log: ${JENKINS_URL_PUBLIC}/job/needapp-be/${BUILD_NUMBER}/console"}'
            """
        }
        failure {
            sh """
                curl -s -X POST '${GG_CHAT_WEBHOOK}' \
                -H 'Content-Type: application/json' \
                -d '{"text": "❌ *NeedApp BE Deploy THAT BAI*\\nBuild: #${BUILD_NUMBER}\\nCommit: ${GIT_COMMIT.take(7)}\\nXem log: ${JENKINS_URL_PUBLIC}/job/needapp-be/${BUILD_NUMBER}/console"}'
            """
        }
        aborted {
            sh """
                curl -s -X POST '${GG_CHAT_WEBHOOK}' \
                -H 'Content-Type: application/json' \
                -d '{"text": "⚠️ *NeedApp BE Deploy BI HUY*\\nBuild: #${BUILD_NUMBER}\\nXem log: ${JENKINS_URL_PUBLIC}/job/needapp-be/${BUILD_NUMBER}/console"}'
            """
        }
    }
}
